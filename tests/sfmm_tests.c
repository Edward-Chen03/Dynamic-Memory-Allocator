...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	...
	// should be 4112 / (heap size) -> Largest Padded Aggregate Payload over Heap Size at Current Time

	double utilizeVal = sf_utilization();

	cr_assert(utilizeVal == ((double) 4128 / PAGE_SZ), "Utilization Value Incorrect! Expected %f but was %f ", ((double) 4128 / PAGE_SZ), utilizeVal);
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	// tests utilization on uninitialized heap

	double utilizeVal = sf_utilization();

	cr_assert(utilizeVal == (0), "Utilization Value Incorrect! Expected %f");

}

Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {

	// tests quicklist flushing

	void* x1 = sf_malloc(sizeof(int));
	void* x2 = sf_malloc(sizeof(int));
	void* x3 = sf_malloc(sizeof(int));
	void* x4 = sf_malloc(sizeof(int));
	void* x5 = sf_malloc(sizeof(int));
	void* x6 = sf_malloc(sizeof(int));
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT, .signal = SIGABRT) {

	// tests NULL invalid free pointer
...
Test(sfmm_student_suite, student_test_7, .timeout = TEST_TIMEOUT) {
	...
	// tests malloc, realloc and free in a single run
	...
	assert_free_block_count(528,1);
	assert_free_block_count(6672,1);
